...
# and `pip install Finance and ML` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "Finance and ML", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n Finance and ML -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "Finance and ML", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "Finance and ML", "YOUR_CONSOLE_SCRIPT"]
